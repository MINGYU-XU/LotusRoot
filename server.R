...
#Proj/Data-----
options(DT.options = list(pageLength = 8)) ## The initial display is 8 rows
...
# ADD proj -----
  # Project ID generated by the system -----
    ...
    autoProjID<- reactive({
      auto_pid <- nrow(projVal())+1
      return(auto_pid)
    })
    output$proj_id <- renderText({  autoProjID()  })
  ...
  observeEvent(input$add_proj,{
    ...
    p <- rbind(data.frame(
      Project.ID = autoProjID(), #nrow(projVal())+1, 
  ...
  autoDataID<- reactive({
    auto_did <- nrow(dataVal())+1
    return(auto_did)
  })
  
  output$data_id <- renderText({  autoDataID()}) #nrow(datas)+1
...
# ADD data -----
  ...
  observeEvent(input$add_data,{
    ...
    t <- rbind(data.frame(
      Data.ID = autoDataID(),#nrow(datas)+1
